...
def LM_rigid(tet_v, tet_f, point_cloud, meshes, PC_lable, queue, liver_model, templates, index):
    ...
    for i in range(80):
        ...
        contribution2displacement = np.reshape(templates, [-1, 3, 69])
        ...
        dEdD = 2 * np.sqrt(alpha) * stiffness.T.dot(np.reshape(deformation, [-1]))
        dEdc = np.matmul(np.expand_dims(dEdD, axis=0), templates)
        #dEdc = np.concatenate([dEdc[:, :4], dEdc[:, 5:]], axis=1)

        jacobian1 = np.concatenate([drdS, drdT, drdR3, drdR2, drdR1], axis=1)
        ...
        increament_rigid = - np.matmul(np.matmul(np.linalg.inv(jTj1 + 0.00*np.diag(jTj1)), jacobian1.T), residual)

        increament_non_rigid = 0.3 * np.matmul(np.matmul(np.linalg.inv(jTj2 + 0.00*np.diag(jTj2)), jacobian2.T), residual)

        #increament_rigid = -1e-2 * np.mean(jacobian1*np.expand_dims(residual, axis=1), axis=0)
        ...
        increament_rigid[0] = 0.0
        if i<20:
            increament = np.concatenate([#np.zeros(shape=[1], dtype=np.float64),
                                         increament_rigid,
                                         0*increament_non_rigid], axis=0)
        else:
            #increament_rigid[0] = 0.0
            increament = np.concatenate([#np.zeros(shape=[1], dtype=np.float64),
                                         0*increament_rigid,
...
def one_model(tet_v, tet_f, meshes, index, liver_model, templates):
    ...
    process.join()
    min = 1'''
    with h5py.File('../../org/datasets/Set' + str(index).zfill(3)+'/nonrigid_without_normal_separate_withoutscale.h5', 'w') as f:
...
def main():
    ...
    min_all = []
    paras = []
    for i in [67, 57, 44, 84, 5, 11, 22, 33, 78, 91, 100, 110]:
...
def generate_cooresponding_points():
    ...
    for i in range(112):
        ...
        closest_point, rotation_matrix, deformation = apply_transform_non_rigid_reverse(S, T, R, C, point_cloud,
                                                                                           deformation)
        print(i+1)
        ...
        with h5py.File('../../org/datasets/Set' + str(i + 1).zfill(3) + '/corresponding_closest_points_on_mesh.h5', 'w') as f:
...
def re_order_templates():
    templates = load_disp_solutions('../displacement_solutions/Y1000000.0_P0.49_23p', 69)
    templates = np.reshape(templates, [-1, 3, 69])
    with h5py.File('../V_order.h5', 'r') as hf:
        order = hf['order'][:]
    #templates = templates[order, :, :]
    templates = np.reshape(templates, [-1, 69])
    return templates
