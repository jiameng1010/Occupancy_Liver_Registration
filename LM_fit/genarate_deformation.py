...
def generate_displacement(mesh, C, mean, pca_vectors, index_in_org):
    ...
    displacement, range = get_displacement_boundary(C_matrix, vertices_sorted, mean, pca_vectors[1], index_in_org)
...
def main():
    ...
    for i in range(np.sum(num_of_combination[:(order_of_polynomial+1)])):
        ...
        #C = C * coefficient
        C[4] = 0.0
        displacement, ranges = generate_displacement(mesh, C, mean, pca_vectors, index_in_org)
        ...
        plot_mesh_vectors(liver_model.simplified_tetr_mesh.vertices, liver_model.simplified_tetr_mesh.faces,
                                liver_model.simplified_tetr_mesh.vertices[back_index_simplified, :], simplified_displacement[back_index_simplified, :])
        mesh_deformed = solve_one_laplacian()
        continue
        simplified_mesh_deformed2 = solve_one_quadratic(liver_model.tetr_mesh,
                                                           liver_model.stiffness,
                                                           laplacian, back_index,
                                                           displacement)
