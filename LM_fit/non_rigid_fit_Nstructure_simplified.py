...
def LM_rigid(tet_v, tet_f, point_cloud, meshes, PC_lable, queue, liver_model, templates, index):
    ...
    for i in range(80):
        ...
        dEdD = 2 * stiffness.T.dot(np.reshape(deformation, [-1]))
        dEdc = np.matmul(np.expand_dims(dEdD, axis=0), templates)
        dEdc = np.concatenate([np.zeros(shape=[1,7], dtype=np.float64), dEdc[:, :4], dEdc[:, 5:]], axis=1)
        ...
        increament = - np.matmul(np.matmul(np.linalg.inv(jTj + 0.5*np.diag(jTj)), jacobian.T), residual_all)
        #increament[7:] = increament[7:]/tet_v.shape[0]
        increament = np.concatenate([increament[:11], np.asarray([0.0]), increament[11:]], axis=0)

        #increament = np.concatenate([increament, np.zeros(shape=14, dtype=np.float32)], axis=0)
        print(parameter)
        parameter = parameter + 0.1*increament
    return min, para
...
def one_model(tet_v, tet_f, meshes, index, liver_model, templates):
    ...
    process.join()
    min = 1'''
    with h5py.File('../../org/datasets/Set' + str(index).zfill(3)+'/non_rigid.h5', 'w') as f:
...
def main():
    ...
    paras = []
    for i in range(112):
        min, para = one_model(liver_model.tetr_mesh.vertices, liver_model.tetr_mesh.faces, meshes, i+1, liver_model, templates)
        min_all.append(min)
...
def plot_reg():
    ...
    for i in range(112):
        with h5py.File('../../org/datasets/Set' + str(i+1).zfill(3) + '/rigid.h5', 'r') as f:
            ...
            f.close()
        with h5py.File('../../org/datasets/Set' + str(i+1).zfill(3) + '/non_rigid.h5', 'r') as f:
            error = f['error'].value
            print(error)
        ...
        print(i+1)
        plot_meshes_n_points(tran_v, liver_model.tetr_mesh.faces,\
                             tran_v_non, liver_model.tetr_mesh.faces,\
                             point_cloud)

    print('done')

def re_order_templates():
    templates = load_disp_solutions('../displacement_solutions/simplified_mesh2', 7)#np.sum(num_of_combination[:4]))
    templates = np.reshape(templates, [-1, 3, 7])
    with h5py.File('../V_order.h5', 'r') as hf:
        order = hf['order'][:]
    #templates = templates[order, :, :]
    templates = np.reshape(templates, [-1, 7])
    return templates
...
def debug():
    ...
    liver_model = LiverModel()
    templates = load_disp_solutions('../displacement_solutions/simplified_mesh2', np.sum(num_of_combination[:4]))
    mesh_liver = trimesh.load('../../org/Liver.off')

    order_reverse = np.argsort(v_order)

    deformation_templates = re_order_templates()
    deformation_templates = np.reshape(deformation_templates, [-1, 3, 7])
    for i in range(14):
        ...
        plot_meshes(liver_model.simplified_tetr_mesh.vertices, liver_model.simplified_tetr_mesh.faces,
                    liver_model.simplified_tetr_mesh.vertices+disp, liver_model.simplified_tetr_mesh.faces)
