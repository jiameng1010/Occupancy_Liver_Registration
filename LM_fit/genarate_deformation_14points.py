...
def find_control_points(liver_model):
    mesh_complement = pymesh.load_mesh('../../org/Liver_surface_14points.off')
...
def generate_displacement(liver_model, mesh, C, mean, pca_vectors):
    ...
    return displacement/60, control_point_index_in_org
...
def compute_displacements():
    ...
    for i in range(np.sum(num_of_combination[:(order_of_polynomial+1)])):
        C = np.zeros(shape=[np.sum(num_of_combination[:(order_of_polynomial+1)])-1])
        C[i] = 1.0
...
def main():
    ...
    for i in range(np.sum(num_of_combination[:(order_of_polynomial+1)])-1):
        C = np.zeros(shape=[np.sum(num_of_combination[:(order_of_polynomial+1)])-1])
        C[i] = 1.0
